...
function incomeFromHomeCountAndYears(initialHomeCount, growthYears, marketConditions) {
  ...
  for (let month = 1; month <= growthYears * 12; month++) {
    ...
    for (const home of homes) {
      ...
      if (growthYears * 12 - month > 24) {
        // if we are within two and a half years of the withdrawl stage we should stop buy homes
        if (home.getPossibleRefinancePayout(month) > currentHomePrice * fractionOfHomePriceToGetIn) {
          ...
          newHomesAddedThisMonth.push(
            new House(
              ...
              loanTermYears,
              refinanceCost,
              homes.length + newHomesAddedThisMonth.length
            )
          );
  ...
  for (let home of homes) {
    ...
    totalEquity +=
      home.getCurrentHomeValue(monthWithdrawlStarts) -
      home.schedule[monthWithdrawlStarts - home.monthOfLatestMortgageOrRefinance].remainingBalance;
